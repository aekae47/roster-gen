    ...
    <div className={cn("min-h-screen transition-all duration-300 font-sans selection:bg-pink-200 dark:selection:bg-pink-900", isDarkMode ? "dark bg-gray-900 text-gray-100" : "bg-slate-50 text-gray-800")}>
      ...
      <nav className="fixed bottom-0 w-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-lg border-t border-gray-200 dark:border-gray-800 p-3 z-50 flex justify-around items-center safe-area-pb">
        <button onClick={() => !isLocked && setShowDocManager(true)} className="group flex flex-col items-center gap-1 text-gray-400 hover:text-blue-600">
          <div className="p-1.5 rounded-full group-hover:bg-blue-50 transition-colors"><UserPlus size={20} /></div><span className="text-[9px] font-bold">Manage Staff</span>
        </button>
        ...
        <button onClick={async () => {
           ...
           <div className="p-1.5 rounded-full group-hover:bg-pink-50 transition-colors"><ImageIcon size={20} /></div><span className="text-[9px] font-bold">Export PNG</span>
        </button>
      </nav>
        ...
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-2xl shadow-2xl flex flex-col max-h-[85vh] border border-gray-100 dark:border-gray-800 relative">
            ...
            {pickingColorFor && (
               <div className="absolute inset-0 z-10 bg-white/95 dark:bg-gray-900/95 rounded-2xl flex flex-col items-center justify-center p-6">
                  ...
                  <div className="grid grid-cols-5 gap-3">
                     {PASTEL_COLORS.map(c => (
                        <button key={c} style={{backgroundColor: c}} className="w-10 h-10 rounded-full border-2 border-transparent hover:scale-110 hover:border-blue-500 shadow-sm transition-all"
                           onClick={() => {
                              setDoctors(doctors.map(d => d.id === pickingColorFor ? {...d, color: c} : d));
                ...
                <div key={catKey} className="space-y-3">
                  ...
                  {doctors.filter(d => d.category === catKey).map(doc => (
                    <div key={doc.id} className="flex gap-2 items-center">
                      <button 
                        onClick={() => setPickingColorFor(doc.id)}
                        className="w-6 h-6 rounded-full shadow-sm border border-black/10 flex-shrink-0 hover:scale-110 transition-transform flex items-center justify-center" 
                        style={{ backgroundColor: doc.color }}>
                          <Palette size={10} className="opacity-50 text-gray-800"/>
                      </button>
                      <input 
                        className="flex-grow text-sm bg-gray-50 dark:bg-gray-800 border-transparent focus:border-blue-400 rounded-lg px-2 py-1.5 outline-none font-medium"
                        value={doc.name}
            ...
            <div className="p-4 border-t border-gray-100 rounded-b-2xl">
              <button onClick={() => { saveToCloud({ doctors }); setShowDocManager(false); }} className="w-full bg-blue-600 text-white py-2.5 rounded-xl font-bold text-xs shadow-lg">SAVE CHANGES</button>
            </div>
          </div>
        </div>
      )}
